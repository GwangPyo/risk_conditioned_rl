...
class MLP(hk.Module):
    def __init__(
        ...
        self.d2rl = d2rl
        self.hidden_kwargs = {"w_init": hk.initializers.Orthogonal(scale=hidden_scale), "with_bias": with_bias}
        self.output_kwargs = {"w_init": hk.initializers.Orthogonal(scale=output_scale), "with_bias": with_bias}
...
class PowerIQNHead(hk.Module):
    ...
    def __call__(self, feature, taus):
        relu_feature = jax.nn.relu(feature)
        # x = self.outputs(relu_feature, taus)
        taus = self.power_embedding(taus)
        qfs = self.monotone(relu_feature[:, None, :] * taus).squeeze(-1)
        return self.outputs(feature, jnp.log(qfs + 1e-8))


@jax.jit
def gaussian_reparameterization(mu, logstd, key):
    std = jnp.exp(logstd)
    return mu + jax.random.normal(key=key, shape=std.shape) * std


@jax.jit
def tanh_normal_reparamterization(mu, logstd, key):
    return jnp.tanh(gaussian_reparameterization(mu, logstd, key))


@jax.jit
def get_actions_logprob(mu, logstd, key):
    std = jnp.exp(logstd)
    gaussian_actions = gaussian_reparameterization(mu, logstd, key)
    actions = jnp.tanh(gaussian_actions)
    log_prob_normal = -0.5 * ((gaussian_actions - mu)/std) ** 2 - logstd - jnp.log(jnp.sqrt(2 * jnp.pi))
    tanh_adjust = jnp.log(1. - jnp.clip(jnp.power(actions, 2), -1 + 1e-7, 1. - 1e-7))
    log_prob = log_prob_normal - tanh_adjust
    return actions, log_prob.sum(axis=-1, keepdims=True)
