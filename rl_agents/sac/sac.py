...
class SAC(OffPolicyPG):
    ...
    def __init__(self,
        ...
        if target_entropy is None:
            self.target_entropy = -np.log(self.env.action_space.shape[0])
        else:
    ...
    def actor_loss(self,
        ...
        qf = jnp.mean(qf, axis=-1, keepdims=True)
    ...
    def critic_loss(self,
        ...
        current_qf = self.critic.apply(param_critic, obs, actions, taus)
    ...
    def sample_taus(self, key):
        taus = jax.random.uniform(key, shape=(self.batch_size, self.n_quantiles))
        return taus
    ...
    def train_step(self):
        ...
        taus, next_taus = self.sample_taus(next(self.rng)), self.sample_taus(next(self.rng))
        ...
        self.opt_critic_state, self.param_critic, qf_loss, _ = optimize(
            ...
            next_obs, taus, next_taus, ent_coef, next(self.rng))
        ...
        self.opt_actor_state, self.param_actor, actor_loss, log_pi = optimize(
            ...
            self.param_actor,
            self.param_critic,
            obs, taus, ent_coef, next(self.rng))
